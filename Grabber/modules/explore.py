...
import random
from . import app, user_collection , collection , add , deduct, show
...
async def random_daily_reward(client, message):
    ...
    if message.reply_to_message:
        await message.reply_text("Fuck Explore a orc den and got 10000 tokens.⚡")
        return
    ...
    if user_id in user_cooldowns and (datetime.utcnow() - user_cooldowns[user_id]) < timedelta(seconds=COOLDOWN_DURATION):
        ...
        await message.reply_text(f"You must wait {int(remaining_time)} seconds before using explore again.")
        return

    user_balance = await show(user_id)  # Assuming `show` function retrieves user balance
    crime_fee = 300

    if user_balance < crime_fee:
        await message.reply_text("Bc You need at least 500 tokens to use explore.")
        return

    await deduct(user_id, crime_fee)  # Assuming `deduct` function deducts tokens from user
    ...
    await add(user_id, random_reward)  # Assuming `add` function adds tokens to user
    last_command_time[user_id] = datetime.utcnow()
    user_cooldowns[user_id] = datetime.utcnow()

    await message.reply_text(f"You {random_message} and got {random_reward} tokens.🤫")

...
async def enter_safari(update, context):
    ...
    await message.reply_text(f"**Welcome to the pick Zone!**\nEntry fee deducted: {entry_fee} Tokens\n\nBegin your /explore for rare slave.")
...
async def throw_ball(callback_query):
    try:
        ...
        if outcome == "🔮🔮🔮":
            ...
            await callback_query.message.edit_caption(caption="**Your contract crystal failed.**\n**The wild slave fled.**", parse_mode="Markdown")
            del sessions[waifu_id]
...
async def run_away(callback_query):
    try:
        ...
        await callback_query.message.edit_caption(caption="**The wild slave has fled.**\n**Try again!**", parse_mode="Markdown")

    except Exception as e:
        logger.error(f"An error occurred in run_away: {e}")
        logger.error(traceback.format_exc())
        await callback_query.answer("An error occurred. Please try again later.", show_alert=True)

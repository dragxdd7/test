...
async def store_handler(_, message):
    ...
    markup = IKM([
        ...
        [IKB("Refresh Store 🔄 (60000 Gold)", callback_data=f"refresh_{user_id}")],
...
async def buy_handler(_, query):
    ...
    markup = IKM([
        ...
        [IKB("Back 🔙", callback_data=f"page_{user_id}_{char_index + 1}")]
    ])
...
async def confirm_handler(_, query):
    ...
    await query.edit_message_caption("Purchase successful! Character added to your collection.")
...
async def refresh_handler(_, query):
    ...
    if not user_data or user_data.get("last_refresh") == today():
        return await query.answer("You can only refresh your store once per day!", show_alert=True)

    gold = user_data.get("gold", 0)
    refresh_cost = 60000
    if gold < refresh_cost:
        return await query.answer("You don't have enough gold to refresh your store!", show_alert=True)

    await clear_user_session(user_id)
    await user_collection.update_one(
        {"id": user_id},
        {"$inc": {"gold": -refresh_cost}, "$set": {"last_refresh": today()}}
    )

    await query.edit_message_caption("Store refreshed! Use /store to view your new store.")

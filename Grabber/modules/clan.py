...
async def create_clan(client, message):
    ...
    try:
        ...
        while await clan_collection.find_one({'clan_id': int(clan_id)}):
            clan_id = str(uuid.uuid4())

        clan_data = {
            'clan_id': int(clan_id),  # Ensure clan_id is integer
            'name': clan_name,
            'leader_id': user_id,
        ...
        await user_collection.update_one({'id': user_id}, {'$set': {'clan_id': int(clan_id)}})
...
async def delete_clan(client, message):
    ...
    await clan_collection.delete_one({'clan_id': int(clan_id)})
    await user_collection.update_many({'clan_id': int(clan_id)}, {'$unset': {'clan_id': ""}})
